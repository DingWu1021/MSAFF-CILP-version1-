...
class MsaffGait6L(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.pretrained_clip = model_cfg["pretrained_clip"]
    ...
    def init_clip(self):
         self.msg_mgr.log_info(f'load model from: {self.pretrained_clip}')
    ...
    def forward(self, inputs):
        ...
        #####################wzx##########################
        with torch.no_grad():
            text_tokens = clip.tokenize([self.text]).cuda()
            text_tokens = text_tokens
            text_tokens = self.model.encode_text(text_tokens)
            text_tokens = text_tokens.to(self.linear.weight.dtype)
        text_tokens = self.linear(text_tokens)
        text_features_reduced = text_tokens.view(1, 1, 256, 1)
        x_3 = x_3 * text_features_reduced
